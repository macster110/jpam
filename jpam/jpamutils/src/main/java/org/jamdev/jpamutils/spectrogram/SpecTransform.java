	...
	public class SpecTransform {
		 ...
		 * @return reference to the spectrogram object.
		 */
		public SpecTransform log10() {
			if (this.specData == null)
				initialiseSpecData();
			this.specData = log10(this.specData);
			if (maintainPhase)
		 ...
		 * Normalise the current spectrogram between the minimum and maximum of the
		 * array
		 * @param min - the minimum value for the normalisation
		 * @param max - the maximum value for the normalisation
		 * @return reference to the normalised spectrogram
		 */
		public SpecTransform normaliseMinMax(double min, double max) {
			if (this.specData == null)
				initialiseSpecData();
			this.specData = normaliseMinMax(this.specData, min, max);
			//		System.out.println("Min max: 2" + JamArr.min(specData)); 
			if (maintainPhase)
				absSpec2Complex(); // set the new data in the complex spectrogram
			return this;
		 ...
		 * @param clipLimit - the clip limit for the histogram equalization.
		 * @param alpha - the alpha parameter for the Rayleigh distribution.
		 * @return the histogram equalized spectrogram.
		 */
		 ...
		 * @return reference to the spectrogram with the value added to all elements.
		 */
		public SpecTransform add(double addVal) {
			if (specData == null)
				initialiseSpecData();
	
			this.specData = add(this.specData, addVal);
		...
		/**
		 * Convert a spectrogram to log10.
		 * @param array - the absolute spectrogram array.
		 * @return the log10 spectrogram.
		 */
		public static double[][] log10(double[][] array) {
			...
			for (int i = 0; i < array.length; i++) {
				for (int j = 0; j < array[i].length; j++) {
					logSpectrgram[i][j] = Math.log10(array[i][j]);
				}
			}
		 ...
		 * @param min - the minimum value for the nomralisation
		 * @param max - the maximum value for the normalisation
		 * @return the normalised spectrogram.
		 */
		public static double[][] normaliseMinMax(double[][] img, double min, double max) {
		...
		public static double[][] normaliseMinMax(double[][] img) {
			...
			double min = JamArr.min(img);
	
			double max = JamArr.max(img);
	
			return normaliseMinMax(img,  min,  max);
		...
		public static double[][] add(double[][] dataF, int xTile, int yTile, double clipLimit, double alpha) {
